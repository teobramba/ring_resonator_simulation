...
lambda_0_gold = 1308.8e-9; % 1549.7e-9;      
f_0_gold = c / lambda_0_gold;
lambda_0_DUT = 1310e-9; %1550e-9;       
f_0_DUT = c / lambda_0_DUT;
lambda_0_LED = 1310e-9; % 1550.5e-9;     
f_0_LED = c / lambda_0_LED;
FSR_gold = 3e12;             B_gold = 5e9;
FSR_DUT = 1.7e12;              B_DUT = 33e9;
ng = 3.74;                     % 3.7 for o band, 4.2 for C band
neff = 2.85;
Ptot = 1e-3;
alpha_db_cm = 2;
% Heating parameters
...
K1_DUT_ideal = K1_DUT_ideal + K1_DUT_ideal*0.15;
...
GC_loss_dB = 7;     % 3dB GC loss at central frequency
...
%% Setup Dither Parameters and power sweep
P_heater_max = 35e-3;
P_sweep = linspace(0, P_heater_max, 100);
...
gap1 = 0;                          % gap distance error for coupler 1 [nm], best -4nm
...
% REMOVED 3f FROM COLORS AND NAMES
colors = {'r', [0.8, 0.8, 0]}; % , 'g'};
...
subplot(4, 4, 5); h_track_TT = plot(NaN, NaN, 'b-', 'LineWidth', 1.5); title('Power Track: TT'); xlabel('Heater (mW)'); ylabel('Output (mW)'); grid on; xlim([0 max(P_sweep)*1000]);
subplot(4, 4, 6); h_track_DD = plot(NaN, NaN, 'b-', 'LineWidth', 1.5); title('Power Track: DD'); xlabel('Heater (mW)'); grid on; xlim([0 max(P_sweep)*1000]);
subplot(4, 4, 7); h_track_DT = plot(NaN, NaN, 'b-', 'LineWidth', 1.5); title('Power Track: DT'); xlabel('Heater (mW)'); grid on; xlim([0 max(P_sweep)*1000]);
subplot(4, 4, 8); h_track_TD = plot(NaN, NaN, 'b-', 'LineWidth', 1.5); title('Power Track: TD'); xlabel('Heater (mW)'); grid on; xlim([0 max(P_sweep)*1000]);

% Row 3: Harmonics
% CHANGED LOOP FROM 1:3 TO 1:2
subplot(4, 4, 9); hold on; for i=1:2, h_harm_TT(i) = plot(NaN, NaN, 'Color', colors{i}, 'LineWidth', 1.5); end; title('Harmonics: TT'); ylabel('Amp (mW)'); grid on; xlim([0 max(P_sweep)*1000]); legend(harmonic_names, 'Location', 'northwest');
subplot(4, 4, 10); hold on; for i=1:2, h_harm_DD(i) = plot(NaN, NaN, 'Color', colors{i}, 'LineWidth', 1.5); end; title('Harmonics: DD'); grid on; xlim([0 max(P_sweep)*1000]);
subplot(4, 4, 11); hold on; for i=1:2, h_harm_DT(i) = plot(NaN, NaN, 'Color', colors{i}, 'LineWidth', 1.5); end; title('Harmonics: DT'); grid on; xlim([0 max(P_sweep)*1000]);
subplot(4, 4, 12); hold on; for i=1:2, h_harm_TD(i) = plot(NaN, NaN, 'Color', colors{i}, 'LineWidth', 1.5); end; title('Harmonics: TD'); grid on; xlim([0 max(P_sweep)*1000]);
% Row 4: Ratios
ratio_limit = [-0.1 1.5]; 
...
for k = 1:length(P_sweep)
    ...
    % --- REMOVED 3rd HARMONIC CALCULATION ---
    ...
    % --- APPLY 2f OFFSET SUBTRACTION AND MULTIPLY BY 2 ---
    if k == 1
        % Capture the very first value of the 2f harmonic to use as offset
        offset_TT_2f = H_TT(2, 1);
        offset_DD_2f = H_DD(2, 1);
        offset_DT_2f = H_DT(2, 1);
        offset_TD_2f = H_TD(2, 1);
    end
    
    % Subtract offset and multiply by 2
    H_TT(2, k) = (H_TT(2, k) - offset_TT_2f) * 2;
    H_DD(2, k) = (H_DD(2, k) - offset_DD_2f) * 2;
    H_DT(2, k) = (H_DT(2, k) - offset_DT_2f) * 2;
    H_TD(2, k) = (H_TD(2, k) - offset_TD_2f) * 2;
...
% compute extintion ratio of integrated power
simPower = [Power_DT' , zeros(size(Power_DT'))];
ER_gold_DUT_power = ext_ratio(simPower);
...
%% --- Convolution Logic ---
% Convolve the Drop port of Gold with Through and Drop of DUT
% Use 'same' to keep the output array exactly 10000 points long for plotting
conv_thru = conv(P_gold_plot(:, 2), P_DUT_plot(:, 1), 'same');
conv_drop = conv(P_gold_plot(:, 2), P_DUT_plot(:, 2), 'same');
% Create the new variable containing both
P_gold_DUT_conv = [conv_thru, conv_drop];

% Optional: Normalize the convolution to a max of 1 so it plots nicely on the same Y-axis
P_gold_DUT_conv(:, 1) = P_gold_DUT_conv(:, 1) / max(P_gold_DUT_conv(:, 1));
P_gold_DUT_conv(:, 2) = P_gold_DUT_conv(:, 2) / max(P_gold_DUT_conv(:, 2));
...
plot(f_plot_gold/1e12, P_gold_plot(:, 1), "Color", [0.7 0.2 0.2], "LineWidth", 2); hold on; grid on
plot(f_plot_gold/1e12, P_gold_plot(:, 2), "Color", [0.2 0.2 0.7], "LineWidth", 2);

plot(f_plot_gold/1e12, P_DUT_plot(:, 1), "Color", [0.9 0.1 0], "LineWidth", 2); % Darker green for visibility
plot(f_plot_gold/1e12, P_DUT_plot(:, 2), "Color", [0 0.2 0.9], "LineWidth", 2);   % Bright green

% Convolution Results (Plotted in Green)
plot(f_plot_gold/1e12, P_gold_DUT_conv(:, 1), "Color", [0.1 0.7 0], "LineWidth", 2); % Darker green for visibility
% plot(f_plot_gold/1e12, P_gold_DUT_conv(:, 2), "Color", [0 1 0], "LineWidth", 2, "LineStyle", "--");   % Bright green

legend({'Through', 'Drop'}, 'Location', 'northwest', 'FontSize', 12);
xlabel('Frequency [THz]', 'FontSize', 14);
ylabel('Transmission', 'FontSize', 14);
title('Transfer Functions: Gold, DUT and Convolution', 'FontSize', 15);

legend({'Gold Through', 'Gold Drop', 'DUT Through', 'DUT Drop', 'Conv (Gold Drop * DUT Thru)'}, ...
    'Location', 'northwest', 'FontSize', 11);
title('DUT Transfer Function & Convolutions', 'FontSize', 15);
xlim([228.6 229.2]);
ylim([0 1.1]);

% % find the center for the crop by hand...
% figure;
% plot(P_gold_DUT_conv(:, 1));

% take only the central part of the convolution
P_gold_DUT_conv_crop = P_gold_DUT_conv(:, 1);
P_gold_DUT_conv_crop = P_gold_DUT_conv_crop(3600:5100); 
% plot the cropped convoluted transfer function
figure;
plot(P_gold_DUT_conv_crop, LineWidth=2);
title('Cropped Convolution of Gold and DUT', 'FontSize', 13);
grid on;

simConv = [P_gold_DUT_conv_crop , zeros(size(P_gold_DUT_conv_crop))];
ER_gold_DUT_conv = ext_ratio(simConv);

% Stamp the Extintion ratio values
fprintf('--- Extinction Ratio ---\n');
fprintf('Extinction Ratio - Gold:     %.2f dB\n', ER_gold);
fprintf('Extinction Ratio - DUT:      %.2f dB\n', ER_DUT);
fprintf('Extinction Ratio - Power:    %.2f dB\n', ER_gold_DUT_power);
fprintf('Extinction Ratio - Convol:   %.2f dB\n', ER_gold_DUT_conv);